<template>
    <article
        class="group relative flex w-full max-w-90 flex-col gap-2 overflow-hidden rounded-lg border border-surface-tone2 bg-surface-overlay py-2 transition-colors hover:border-surface-tone hover:bg-surface-raised"
    ...
    >
        ...
        <header class="relative z-10 flex items-center justify-between gap-2 px-4">
            ...
            <span class="shrink-0 text-center text-xs font-bold leading-6 text-text-body">
                {{ roundLabel || 'الجولة الأولى' }}
            </span>

            <span
                class="min-w-0 flex-1 text-end text-xs leading-6"
                :class="isUpcoming ? 'text-[#E2A330]' : 'text-text-body'"
            >
                {{ statusLabel }}
            </span>
        </header>
