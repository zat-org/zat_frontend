# Graph Report - zat_frontend  (2026-08-10)

## Corpus Check
- 240 files · ~458,895 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1308 nodes · 1760 edges · 155 communities (134 shown, 21 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 5 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `11c755cb`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- [[_COMMUNITY_Community 0|Community 0]]
- [[_COMMUNITY_Community 1|Community 1]]
- [[_COMMUNITY_Community 2|Community 2]]
- [[_COMMUNITY_Community 3|Community 3]]
- [[_COMMUNITY_Community 4|Community 4]]
- [[_COMMUNITY_Community 5|Community 5]]
- [[_COMMUNITY_Community 6|Community 6]]
- [[_COMMUNITY_Community 7|Community 7]]
- [[_COMMUNITY_Community 8|Community 8]]
- [[_COMMUNITY_Community 9|Community 9]]
- [[_COMMUNITY_Community 10|Community 10]]
- [[_COMMUNITY_Community 11|Community 11]]
- [[_COMMUNITY_Community 12|Community 12]]
- [[_COMMUNITY_Community 13|Community 13]]
- [[_COMMUNITY_Community 14|Community 14]]
- [[_COMMUNITY_Community 15|Community 15]]
- [[_COMMUNITY_Community 16|Community 16]]
- [[_COMMUNITY_Community 17|Community 17]]
- [[_COMMUNITY_Community 18|Community 18]]
- [[_COMMUNITY_Community 19|Community 19]]
- [[_COMMUNITY_Community 20|Community 20]]
- [[_COMMUNITY_Community 21|Community 21]]
- [[_COMMUNITY_Community 22|Community 22]]
- [[_COMMUNITY_Community 23|Community 23]]
- [[_COMMUNITY_Community 24|Community 24]]
- [[_COMMUNITY_Community 25|Community 25]]
- [[_COMMUNITY_Community 26|Community 26]]
- [[_COMMUNITY_Community 27|Community 27]]
- [[_COMMUNITY_Community 28|Community 28]]
- [[_COMMUNITY_Community 29|Community 29]]
- [[_COMMUNITY_Community 30|Community 30]]
- [[_COMMUNITY_Community 31|Community 31]]
- [[_COMMUNITY_Community 32|Community 32]]
- [[_COMMUNITY_Community 33|Community 33]]
- [[_COMMUNITY_Community 34|Community 34]]
- [[_COMMUNITY_Community 35|Community 35]]
- [[_COMMUNITY_Community 36|Community 36]]
- [[_COMMUNITY_Community 37|Community 37]]
- [[_COMMUNITY_Community 38|Community 38]]
- [[_COMMUNITY_Community 39|Community 39]]
- [[_COMMUNITY_Community 40|Community 40]]
- [[_COMMUNITY_Community 41|Community 41]]
- [[_COMMUNITY_Community 42|Community 42]]
- [[_COMMUNITY_Community 43|Community 43]]
- [[_COMMUNITY_Community 44|Community 44]]
- [[_COMMUNITY_Community 45|Community 45]]
- [[_COMMUNITY_Community 46|Community 46]]
- [[_COMMUNITY_Community 47|Community 47]]
- [[_COMMUNITY_Community 48|Community 48]]
- [[_COMMUNITY_Community 49|Community 49]]
- [[_COMMUNITY_Community 50|Community 50]]
- [[_COMMUNITY_Community 51|Community 51]]
- [[_COMMUNITY_Community 52|Community 52]]
- [[_COMMUNITY_Community 53|Community 53]]
- [[_COMMUNITY_Community 54|Community 54]]
- [[_COMMUNITY_Community 55|Community 55]]
- [[_COMMUNITY_Community 56|Community 56]]
- [[_COMMUNITY_Community 57|Community 57]]
- [[_COMMUNITY_Community 58|Community 58]]
- [[_COMMUNITY_Community 59|Community 59]]
- [[_COMMUNITY_Community 60|Community 60]]
- [[_COMMUNITY_Community 61|Community 61]]
- [[_COMMUNITY_Community 62|Community 62]]
- [[_COMMUNITY_Community 63|Community 63]]
- [[_COMMUNITY_Community 64|Community 64]]
- [[_COMMUNITY_Community 66|Community 66]]
- [[_COMMUNITY_Community 67|Community 67]]
- [[_COMMUNITY_Community 69|Community 69]]
- [[_COMMUNITY_Community 70|Community 70]]
- [[_COMMUNITY_Community 71|Community 71]]
- [[_COMMUNITY_Community 72|Community 72]]
- [[_COMMUNITY_Community 73|Community 73]]
- [[_COMMUNITY_Community 74|Community 74]]
- [[_COMMUNITY_Community 75|Community 75]]
- [[_COMMUNITY_Community 76|Community 76]]
- [[_COMMUNITY_Community 77|Community 77]]
- [[_COMMUNITY_Community 79|Community 79]]
- [[_COMMUNITY_Community 80|Community 80]]
- [[_COMMUNITY_Community 81|Community 81]]
- [[_COMMUNITY_Community 82|Community 82]]
- [[_COMMUNITY_Community 83|Community 83]]
- [[_COMMUNITY_Community 84|Community 84]]
- [[_COMMUNITY_Community 85|Community 85]]
- [[_COMMUNITY_Community 86|Community 86]]
- [[_COMMUNITY_Community 87|Community 87]]
- [[_COMMUNITY_Community 88|Community 88]]
- [[_COMMUNITY_Community 89|Community 89]]
- [[_COMMUNITY_Community 90|Community 90]]
- [[_COMMUNITY_Community 92|Community 92]]
- [[_COMMUNITY_Community 93|Community 93]]
- [[_COMMUNITY_Community 94|Community 94]]
- [[_COMMUNITY_Community 95|Community 95]]
- [[_COMMUNITY_Community 96|Community 96]]
- [[_COMMUNITY_Community 97|Community 97]]
- [[_COMMUNITY_Community 98|Community 98]]
- [[_COMMUNITY_Community 100|Community 100]]
- [[_COMMUNITY_Community 101|Community 101]]
- [[_COMMUNITY_Community 102|Community 102]]
- [[_COMMUNITY_Community 103|Community 103]]
- [[_COMMUNITY_Community 104|Community 104]]
- [[_COMMUNITY_Community 105|Community 105]]
- [[_COMMUNITY_Community 107|Community 107]]
- [[_COMMUNITY_Community 108|Community 108]]
- [[_COMMUNITY_Community 109|Community 109]]
- [[_COMMUNITY_Community 110|Community 110]]
- [[_COMMUNITY_Community 111|Community 111]]
- [[_COMMUNITY_Community 112|Community 112]]
- [[_COMMUNITY_Community 113|Community 113]]
- [[_COMMUNITY_Community 114|Community 114]]
- [[_COMMUNITY_Community 115|Community 115]]
- [[_COMMUNITY_Community 116|Community 116]]
- [[_COMMUNITY_Community 117|Community 117]]
- [[_COMMUNITY_Community 118|Community 118]]
- [[_COMMUNITY_Community 119|Community 119]]
- [[_COMMUNITY_Community 120|Community 120]]
- [[_COMMUNITY_Community 121|Community 121]]
- [[_COMMUNITY_Community 125|Community 125]]
- [[_COMMUNITY_Community 127|Community 127]]
- [[_COMMUNITY_Community 128|Community 128]]
- [[_COMMUNITY_Community 129|Community 129]]
- [[_COMMUNITY_Community 130|Community 130]]
- [[_COMMUNITY_Community 131|Community 131]]
- [[_COMMUNITY_Community 160|Community 160]]
- [[_COMMUNITY_Community 166|Community 166]]

## God Nodes (most connected - your core abstractions)
1. `FetchFactory` - 32 edges
2. `IChamp` - 23 edges
3. `IMatchLessDetails` - 23 edges
4. `MatchState` - 21 edges
5. `IMatchFullDetails` - 18 edges
6. `ChampionsModule` - 16 edges
7. `MatchesModule` - 15 edges
8. `IApiInstance` - 13 edges
9. `ICupMatchData` - 12 edges
10. `displayTextValue()` - 12 edges

## Surprising Connections (you probably didn't know these)
- `IApiInstance` --references--> `ChampionsModule`  [EXTRACTED]
  types/nuxt.d.ts → features/championships/api/champion.ts
- `IApiInstance` --references--> `FollowersChampionsModule`  [EXTRACTED]
  types/nuxt.d.ts → features/followers/api/followersChampions.ts
- `IApiInstance` --references--> `MatchesModule`  [EXTRACTED]
  types/nuxt.d.ts → features/matches/api/matches.ts
- `IApiInstance` --references--> `AuthModule`  [EXTRACTED]
  types/nuxt.d.ts → features/auth/api/auth.ts
- `IApiInstance` --references--> `BlogsModule`  [EXTRACTED]
  types/nuxt.d.ts → features/blogs/api/blogs.ts

## Import Cycles
- None detected.

## Communities (155 total, 21 thin omitted)

### Community 0 - "Community 0"
Cohesion: 0.15
Nodes (11): { $api }, currentDate, { error, pending, send: sendJoinRequest }, IsPlayedBeforeOptions, maxBirthDate, minBirthDate, props, router (+3 more)

### Community 1 - "Community 1"
Cohesion: 0.16
Nodes (13): ChampionsModule, ChampType, IGetAllChampionsResponse, IGetChampDetailsResponse, IGetChampMatchesResponse, IGetChampStudiosResponse, IGetChampSummaryResponse, IGetChampSummaryWinnerResponse (+5 more)

### Community 2 - "Community 2"
Cohesion: 0.04
Nodes (46): dependencies, apexcharts, chart.js, motion-v, @nuxt/content, @nuxtjs/strapi, pinia, @pinia/nuxt (+38 more)

### Community 3 - "Community 3"
Cohesion: 0.20
Nodes (7): cardBoxRef, preferredReducedMotion, props, reduceMotion, shineRef, titleStyle, ChampionshipTypeConfig

### Community 4 - "Community 4"
Cohesion: 0.08
Nodes (24): activeAccordion, { $api }, champName, countdownParts, displayMatch, endedMatches, error, latestMatch (+16 more)

### Community 5 - "Community 5"
Cohesion: 0.60
Nodes (4): clamp(), decrease(), increase(), model

### Community 6 - "Community 6"
Cohesion: 0.12
Nodes (18): allowDismiss, { $api }, countdown, { error, pending, requestOtp, confirmOtp }, FormState, handleClose(), isLoginFormOpened, onOpenChange() (+10 more)

### Community 7 - "Community 7"
Cohesion: 0.13
Nodes (14): props, rows, selected, selectedId, IStatistics, IStatisticViewData, StatisticOptionName, StatisticOptions (+6 more)

### Community 8 - "Community 8"
Cohesion: 0.24
Nodes (10): mapEstimationItem(), mediaPath(), StrapiEstimationAttributes, StrapiRelation, unwrapRelation(), ICreateMatchEstimation, IEstimationRelation, IEstimationTable (+2 more)

### Community 9 - "Community 9"
Cohesion: 0.15
Nodes (9): bestPlayerFromMatch, bestPlayerId, bestPlayerImage, bestPlayerName, isOpen, props, team1Score, team2Score (+1 more)

### Community 10 - "Community 10"
Cohesion: 0.18
Nodes (7): FollowersChampionsModule, StrapiFollowerChampionAttributes, IFollowerChampion, IFollowerChampionApplyRequest, IMeta, IPagination, IResponse

### Community 11 - "Community 11"
Cohesion: 0.22
Nodes (13): getStrapiMediaAltText(), getStrapiMediaAttributes(), getStrapiMediaItem(), getStrapiMediaLargePath(), getStrapiMediaPath(), mapHeroSlides(), MediaUrlFn, StrapiMediaAttributes (+5 more)

### Community 12 - "Community 12"
Cohesion: 0.27
Nodes (7): mapClientImages(), mapPlayerImages(), unwrapStrapiCounter(), IClientImage, IPlayerImage, IZatStatistics, IYoutubeChannelData

### Community 13 - "Community 13"
Cohesion: 0.10
Nodes (22): { $api }, bestPlayerOptions, canGoNext, emit, { error, pending, send: sendEstimation }, getWinner, goNext(), handleClose() (+14 more)

### Community 14 - "Community 14"
Cohesion: 0.07
Nodes (31): dateParts, isDone, isUpcoming, props, statusLabel, team1IsWinner, team2IsWinner, activeBreakpoints (+23 more)

### Community 15 - "Community 15"
Cohesion: 0.22
Nodes (7): detailStats, endMonth, endYear, mobileWdlPills, props, stats, wdlStats

### Community 16 - "Community 16"
Cohesion: 0.18
Nodes (10): dateSource, displayDate, displayDateIso, props, statusClass, statusLabel, typeConfig, typeEmblem (+2 more)

### Community 17 - "Community 17"
Cohesion: 0.09
Nodes (20): { $api }, champLogoSrc, countdown, featuredMatch, hasUpcomingEstimation, hasWinner, matches, pointsDisplay (+12 more)

### Community 18 - "Community 18"
Cohesion: 0.09
Nodes (18): activeMatch, activeTab, { $api }, countdown, endedMatches, error, hasContent, matchDate (+10 more)

### Community 19 - "Community 19"
Cohesion: 0.17
Nodes (7): emptyClass, groupHeaderClass, headerClass, isUpcoming, listClass, openChampId, props

### Community 20 - "Community 20"
Cohesion: 0.12
Nodes (13): { $api }, emit, { error: errorAtSendApplyRequest, pending: sendApplyRequestPending, send: sendApplyJobRequest }, fileInputRef, formFieldUi, inputUi, onSubmit(), props (+5 more)

### Community 21 - "Community 21"
Cohesion: 0.10
Nodes (20): { $api }, checkEstimationStatus(), { data: estimationData, getData: getUserEstimation }, estimationScore, estimationStatus, fetchFullMatchData(), fullMatchData, handleEstimationSubmitted() (+12 more)

### Community 22 - "Community 22"
Cohesion: 0.21
Nodes (3): MatchesModule, IMatchHistoryParams, IMatchHistoryResponse

### Community 23 - "Community 23"
Cohesion: 0.12
Nodes (14): { $api }, countdown, { error, pending, requestOtp, confirmOtp }, formState, inputUi, onSubmitUsername(), otpSchema, resendDisabled (+6 more)

### Community 24 - "Community 24"
Cohesion: 0.13
Nodes (13): { $api }, cardChamps, cardsType, champs, emptyCardsMessage, emptyListMessage, historyParams, isUpcoming (+5 more)

### Community 25 - "Community 25"
Cohesion: 0.33
Nodes (5): IMatchHistoryChamp, IMatchHistoryGroup, IMatchHistoryMatch, IMatchHistoryTeam, MatchHistoryStatus

### Community 26 - "Community 26"
Cohesion: 0.23
Nodes (9): analystsLabel, displayDate, displayDateIso, props, thumbnail, props, thumbnail, extractYoutubeVideoId() (+1 more)

### Community 27 - "Community 27"
Cohesion: 0.22
Nodes (9): { $api }, champs, champType, pageTitle, props, typeConfig, getChampionshipTypeConfig(), getChampionshipTypeLabel() (+1 more)

### Community 28 - "Community 28"
Cohesion: 0.15
Nodes (13): day, displayPoints(), isUpcoming, matchDate, month, pointsValue(), props, showScoreHints (+5 more)

### Community 29 - "Community 29"
Cohesion: 0.15
Nodes (13): { $api }, { error, pending, send: sendContactUsMessage }, formFieldUi, inputUi, MessagesTypes, onSubmit(), resetState(), router (+5 more)

### Community 30 - "Community 30"
Cohesion: 0.10
Nodes (12): ui, props, LeagueState, IChamp, IRecentChamp, props, { $api }, matches (+4 more)

### Community 31 - "Community 31"
Cohesion: 0.15
Nodes (13): { $api }, currentDate, emit, formFieldUi, inputUi, judgedBeforeItems, maxBirthDate, minBirthDate (+5 more)

### Community 32 - "Community 32"
Cohesion: 0.15
Nodes (10): { $api }, featuredJobs, indicator, jobs, listJobs, pageNumber, pgNumStr, route (+2 more)

### Community 33 - "Community 33"
Cohesion: 0.17
Nodes (11): AppButtonSize, AppButtonVariant, isNativeButton, NuxtLink, props, resolvedColorClass, rootAttrs, rootComponent (+3 more)

### Community 34 - "Community 34"
Cohesion: 0.18
Nodes (11): attrs, classAttr, emit, fallbackIconClass, hasError, hasExplicitSize, hasObjectFit, imgClass (+3 more)

### Community 35 - "Community 35"
Cohesion: 0.12
Nodes (13): { $api }, currentTeamId, currentTeamLogo, currentTeamName, latestTransfer, NuxtLink, player, playerNameEl (+5 more)

### Community 36 - "Community 36"
Cohesion: 0.33
Nodes (4): colorMode, isDark, isSidebarOpen, userStore

### Community 37 - "Community 37"
Cohesion: 0.18
Nodes (8): { $api }, champs, emptyMessage, historyParams, moreLink, timeTab, typeTab, typeTabs

### Community 38 - "Community 38"
Cohesion: 0.29
Nodes (6): useMediaUrl(), StrapiFindParams, useStrapiCollection(), useClientImages(), useHeroSlides(), usePlayerImages()

### Community 39 - "Community 39"
Cohesion: 0.40
Nodes (4): Development Server, Nuxt 3 Minimal Starter, Production, Setup

### Community 40 - "Community 40"
Cohesion: 0.18
Nodes (7): { $api }, cards, siteVisits, twitchSubs, visitCount, youtubeViews, zatChannel

### Community 41 - "Community 41"
Cohesion: 0.15
Nodes (10): NuxtLink, playerTo, props, roleLabel, bestPlayerId, team1Score, team2Score, IMatchFullDetails (+2 more)

### Community 42 - "Community 42"
Cohesion: 0.20
Nodes (6): imageErrors, isExpanded, isHovered, playerSlots, props, showMembersPeek

### Community 43 - "Community 43"
Cohesion: 0.17
Nodes (10): { $api }, pending, previewMatches, previewStudios, props, sectionError, IAnalyst, IStudio (+2 more)

### Community 44 - "Community 44"
Cohesion: 0.11
Nodes (16): props, rows, props, rows, ScoreRow, IChampSummaryWinner, IChampWinnerStats, displayAbnatValue() (+8 more)

### Community 45 - "Community 45"
Cohesion: 0.24
Nodes (9): availableNavigation, champId, champType, isActive(), navigation, normalizePath(), props, route (+1 more)

### Community 46 - "Community 46"
Cohesion: 0.20
Nodes (8): { $api }, { data, error, pending, getData: GetUserEstimationForMatch }, isEstimationFormOpened, isLoginFormOpened, isMatchOpenForEstimations, props, userStore, userSubmittedEstimationCount

### Community 47 - "Community 47"
Cohesion: 0.20
Nodes (9): dateSource, displayDate, displayDateIso, props, statusClass, statusLabel, typeConfig, typeEmblem (+1 more)

### Community 48 - "Community 48"
Cohesion: 0.20
Nodes (8): { $api }, blogs, indicator, pageNumber, pgNumStr, route, router, totalBlogsCount

### Community 49 - "Community 49"
Cohesion: 0.20
Nodes (8): { $api }, indicator, jobs, pageNumber, pgNumStr, route, router, totalJobsCount

### Community 50 - "Community 50"
Cohesion: 0.40
Nodes (4): StrapiEntity, StrapiHeroSlideAttributes, FALLBACK_SLIDES, IHeroSlide

### Community 51 - "Community 51"
Cohesion: 0.40
Nodes (4): moreHref, props, rows, IEstimationRecord

### Community 52 - "Community 52"
Cohesion: 0.22
Nodes (7): hasContent, Member, props, refereeMembers, refereeRoles, team1Members, team2Members

### Community 53 - "Community 53"
Cohesion: 0.28
Nodes (7): emit, fileInput, filename, handleCancelSelection(), handleFileSelection(), props, textInput

### Community 54 - "Community 54"
Cohesion: 0.24
Nodes (7): props, seasonYear, props, rows, IChampLessDetails, IPlayerFullDetails, ITeamStatistics

### Community 55 - "Community 55"
Cohesion: 0.22
Nodes (8): { $api }, blog, publishedDate, publishedIso, publishedLabel, relatedBlogs, relatedPage, route

### Community 56 - "Community 56"
Cohesion: 0.22
Nodes (8): { $api }, columns, estimationResults, page, props, rows, userRow, userStore

### Community 57 - "Community 57"
Cohesion: 0.25
Nodes (6): { $api }, currentPage, pagedStudios, props, sortedStudios, totalPages

### Community 58 - "Community 58"
Cohesion: 0.33
Nodes (4): CHAMPIONSHIP_CARD_SHARED, CHAMPIONSHIP_TYPE_CARDS, CHAMPIONSHIP_TYPE_SLUGS, EXTRA_TYPE_TITLES

### Community 59 - "Community 59"
Cohesion: 0.25
Nodes (5): imageBoxRef, isHovered, preferredReducedMotion, reduceMotion, shineRef

### Community 60 - "Community 60"
Cohesion: 0.12
Nodes (17): HezamTeamSummary, LeagueTeamSummary, TeamSummary, { $api }, champType, estimationRecords, logosById, pending (+9 more)

### Community 61 - "Community 61"
Cohesion: 0.33
Nodes (5): { $api }, champs, champType, route, sectionTitle

### Community 62 - "Community 62"
Cohesion: 0.25
Nodes (7): { height }, isSupported, props, slidesPerView, spaceBetween, SwiperElement, swiperElm

### Community 63 - "Community 63"
Cohesion: 0.33
Nodes (5): hasEstimationWindow, isSmUp, open, props, timerSize

### Community 66 - "Community 66"
Cohesion: 0.29
Nodes (5): paddedHours, paddedMinutes, paddedSeconds, props, sizeClasses

### Community 67 - "Community 67"
Cohesion: 0.29
Nodes (6): isFreeAgent, label, logoFailed, logoSrc, props, teamName

### Community 69 - "Community 69"
Cohesion: 0.33
Nodes (5): { $api }, champs, champType, moreLink, props

### Community 70 - "Community 70"
Cohesion: 0.33
Nodes (4): durationSec, trackLogos, trackRef, viewportRef

### Community 71 - "Community 71"
Cohesion: 0.33
Nodes (5): { $api }, twitch, visitCount, zatChannel, zatPlusChannel

### Community 72 - "Community 72"
Cohesion: 0.33
Nodes (4): activeIndex, colorMode, images, logoSrc

### Community 73 - "Community 73"
Cohesion: 0.33
Nodes (4): colorMode, isDark, isHeaderOpen, { items }

### Community 74 - "Community 74"
Cohesion: 0.31
Nodes (6): { $api }, userStore, emit, logout(), userStore, useUserStore

### Community 77 - "Community 77"
Cohesion: 0.20
Nodes (9): elapsed, matchesCount, matchesLabel, props, toMediaUrl, getElapsedTime(), ChampWinText, props (+1 more)

### Community 79 - "Community 79"
Cohesion: 0.33
Nodes (5): champsCount, props, winsCount, winsDisplay, yearsSinceFounded

### Community 80 - "Community 80"
Cohesion: 0.33
Nodes (5): props, showWins, winsCount, winsDisplay, ITeamLessDetails

### Community 81 - "Community 81"
Cohesion: 0.33
Nodes (5): crumbs, headerLabel, props, resolvedBackLabel, resolvedBackTo

### Community 82 - "Community 82"
Cohesion: 0.33
Nodes (3): props, sortedPlayers, IPlayerLessDetails

### Community 83 - "Community 83"
Cohesion: 0.22
Nodes (7): NuxtLink, allTransfers, hasMore, isModalOpen, previewTransfers, props, ITransfer

### Community 84 - "Community 84"
Cohesion: 0.33
Nodes (5): jobId, jobParam, jobPath, route, router

### Community 85 - "Community 85"
Cohesion: 0.33
Nodes (5): { $api }, jobParam, otherJobs, relatedPage, route

### Community 86 - "Community 86"
Cohesion: 0.17
Nodes (10): cardStatusClass, hasEstimationWindow, isDone, isLive, isUpcoming, kickoffDate, kickoffDateObj, kickoffTime (+2 more)

### Community 88 - "Community 88"
Cohesion: 0.19
Nodes (7): open, emit, onSelect(), open, props, useChampMatchNeighbors(), IMatchLessDetails

### Community 89 - "Community 89"
Cohesion: 0.40
Nodes (4): legalLinks, quickLinks, socialLinks, tournamentLinks

### Community 90 - "Community 90"
Cohesion: 0.50
Nodes (3): emit, props, updateModelValue()

### Community 92 - "Community 92"
Cohesion: 0.40
Nodes (4): { $api }, isMatchDetail, isOverview, route

### Community 93 - "Community 93"
Cohesion: 0.40
Nodes (4): { $api }, champId, matchId, route

### Community 94 - "Community 94"
Cohesion: 0.40
Nodes (3): courseHighlights, generalRules, tableJudgeRules

### Community 95 - "Community 95"
Cohesion: 0.50
Nodes (3): items, props, ui

### Community 96 - "Community 96"
Cohesion: 0.50
Nodes (3): heightClass, pairCount, props

### Community 97 - "Community 97"
Cohesion: 0.50
Nodes (3): attrs, data, masks

### Community 98 - "Community 98"
Cohesion: 0.50
Nodes (3): { $api }, champName, matches

### Community 100 - "Community 100"
Cohesion: 0.67
Nodes (3): emit, props, updateModelValue()

### Community 101 - "Community 101"
Cohesion: 0.50
Nodes (3): selected, uiRadio, uiRadioGroup

### Community 102 - "Community 102"
Cohesion: 0.67
Nodes (3): emit, props, updateModelValue()

### Community 104 - "Community 104"
Cohesion: 0.50
Nodes (3): DropdownChild, groupedRows, props

### Community 105 - "Community 105"
Cohesion: 0.50
Nodes (3): availablePages, emit, props

### Community 107 - "Community 107"
Cohesion: 0.50
Nodes (3): activeId, navItems, values

### Community 108 - "Community 108"
Cohesion: 0.25
Nodes (5): currentPage, pagedMatches, props, sortedMatches, totalPages

### Community 109 - "Community 109"
Cohesion: 0.50
Nodes (3): { $api }, champion, route

### Community 110 - "Community 110"
Cohesion: 0.50
Nodes (3): { $api }, route, team

### Community 120 - "Community 120"
Cohesion: 0.20
Nodes (8): { $api }, isOpen, pagedReferees, pageNumber, pgNumStr, referees, route, router

### Community 160 - "Community 160"
Cohesion: 0.17
Nodes (9): nameClass, props, scoreClass, scoreDisplay, teamName, ITeamData, isMatchOpenForEstimations, props (+1 more)

### Community 166 - "Community 166"
Cohesion: 0.05
Nodes (37): AuthModule, IError, ILoginConfirmed, IUser, BlogsModule, props, publishedDate, publishedIso (+29 more)

## Knowledge Gaps
- **737 isolated node(s):** `nuxt`, `nuxt-ui`, `{ $api }`, `userStore`, `props` (+732 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **21 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `IMatchLessDetails` connect `Community 88` to `Community 160`, `Community 1`, `Community 4`, `Community 9`, `Community 43`, `Community 108`, `Community 17`, `Community 18`, `Community 21`, `Community 86`, `Community 22`, `Community 63`?**
  _High betweenness centrality (0.064) - this node is a cross-community bridge._
- **Why does `FetchFactory` connect `Community 166` to `Community 1`, `Community 8`, `Community 10`, `Community 12`, `Community 22`, `Community 88`?**
  _High betweenness centrality (0.045) - this node is a cross-community bridge._
- **Why does `MatchState` connect `Community 160` to `Community 4`, `Community 41`, `Community 43`, `Community 44`, `Community 14`, `Community 17`, `Community 18`, `Community 19`, `Community 86`, `Community 88`, `Community 25`, `Community 28`?**
  _High betweenness centrality (0.033) - this node is a cross-community bridge._
- **What connects `nuxt`, `nuxt-ui`, `{ $api }` to the rest of the system?**
  _737 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Community 2` be split into smaller, more focused modules?**
  _Cohesion score 0.0425531914893617 - nodes in this community are weakly interconnected._
- **Should `Community 4` be split into smaller, more focused modules?**
  _Cohesion score 0.07816091954022988 - nodes in this community are weakly interconnected._
- **Should `Community 6` be split into smaller, more focused modules?**
  _Cohesion score 0.12105263157894737 - nodes in this community are weakly interconnected._