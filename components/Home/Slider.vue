<template>
    <FetchDataWrapper :error="error ? 'تعذر تحميل البيانات برجاء المحاولة لاحقا' : null" :pending="pending">
        <section v-if="champs && champs.length > 0" aria-label="عرض شرائح زات" class="mb-8 py-8">
            <SectionHeader title="أحدث البطولات" icon="i-heroicons-trophy" />
            <div class=" overflow-hidden h-[200px]">
                <swiper-container class="w-full h-full" :loop="true" :slides-per-view="1" :space-between="30"
                    ...
                    <swiper-slide v-for="champ in champs" :key="champ.id">
                        <div class="h-[200px] bg-center bg-cover bg-surface-secondary rounded-zat-md flex justify-evenly items-center relative overflow-hidden"
                            ...
                            <UButton :to="`/championships/${champ.id}`" variant="outline" class="absolute top-3 right-3"
                                size="xs" trailing-icon="i-heroicons-chevron-double-left-16-solid">
                                اذهب للبطولة
                            </UButton>
                        </div>
                    </swiper-slide>
