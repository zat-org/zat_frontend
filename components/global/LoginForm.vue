    ...
    <UModal v-model="isLoginFormOpened" prevent-close>
        <UCard>
            ...
            <UForm :schema="usernameSchema" :state="state" class="space-y-3" @submit="onSubmitUsername">
                <FormInputField :disabled="state.formState !== FormState.waitUsernameInput" dir="ltr" v-model="state.username" type="text" name="username" label="يوزر قيدها" hint="مطلوب" icon="i-heroicons-at-symbol-20-solid" placeholder="username" />
                ...
                <div v-if="error && state.formState === FormState.waitUsernameInput" class="text-red-500 text-sm flex items-center">
                    <UIcon name="i-heroicons-x-circle" class="text-lg me-1"/>
                    <p>{{ error }}</p>
                </div>
                <div class="flex justify-end">
                    <UButton v-if="state.formState === FormState.waitUsernameInput" :loading="pending" type="submit" icon="i-heroicons-paper-airplane-16-solid">ارسال الرمز</UButton>
                </div>
            </UForm>

            <UForm :schema="otpSchema" v-if="state.formState === FormState.waitOtpInput" :state="state" class="space-y-3" @submit="onSubmitOtp">
               <FormInputField  dir="ltr" v-model="state.otp" type="text" name="otp" label="رمز الدخول" hint="مطلوب" icon="i-heroicons-key" placeholder="123456" />
               <div v-if="error && state.formState === FormState.waitOtpInput" class="text-red-500 text-sm flex items-center">
                    <UIcon name="i-heroicons-x-circle" class="text-lg me-1"/>
                    <p>{{ error }}</p>
                </div>
               <div class="flex justify-end">
